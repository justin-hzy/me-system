...
public class PurOrderServiceImpl implements PurOrderService {
    ...
    public JsonResult transPurOrderDtl(ThRefund thRefund) throws IOException {
        ...
        if("OK".equals(head)){
            ...
            if (CollUtil.isNotEmpty(purchaseOrdersJsonArr)){
                JSONObject purchaseOrderJson = purchaseOrdersJsonArr.getJSONObject(0);
                JSONObject headerJson = purchaseOrderJson.getJSONObject("Header");
                String status = headerJson.getString("Status");

                if("3900".equals(status)){
                    JSONArray jsonArray = purchaseOrderJson.getJSONArray("Items");

                    String erpOrder = purchaseOrderJson.getString("ErpOrder");
    ...
    public JsonResult transPurOrderDtl(ThRefund thRefund,String dtlTableName) throws IOException {
        JSONObject purOrderDtlJson = jsonService.createPurOrderDtlJson(thRefund);
